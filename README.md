...
| **🤖 AI 智能助手** | 通过 Lexi 内置 AI 生成建议、分析、计划与写作反馈；模型与可用范围由管理员统一维护 |
...
点击上方链接即可在线预览。基础记录可直接在本机使用；登录并确认数据归属后，计划、练习、模考和考试日期会自动同步，Lexi 内置 AI 也可使用。
...
3. 发起功能明确的 AI 请求。模型、服务商和调用策略由 Lexi 管理员统一维护；浏览器不会获取 provider Key、真实服务地址或模型路由。
...
| **AI 集成** | Lexi AI Gateway | 服务端受管模型、严格结构化结果与安全渲染 |
...
- 所有学习操作先写入 `localStorage`，断网时仍可正常使用
- 登录并确认归属后，计划、计划执行、练习、模考和考试日期会以小型白名单载荷同步；日记与 AI 正文仍只保存在本机
- 支持正式业务数据的 JSON 导出/导入；私密连接信息、恢复 journal 与影子账本不导出，旧备份会安全迁移每日打卡幂等记录
